The error `TypeError: can only concatenate str (not "float") to str` happens because the "Wall Street" styling inserted empty text (`""`) into the math columns to create visual headers (like "Operating Activities"). When Python tried to calculate the total `cf.sum()`, it crashed trying to add text to numbers.

I have rewritten the entire `dashboard.py` to fix this by using `None` (which represents "Nothing" in math terms) instead of `""` (Empty Text) for headers. I also upgraded the math to be explicit (`A + B + C`) rather than a blind sum.

### The Fixed "Master" `dashboard.py`
...
Version: 3.1 (Stable Financials & Optimization)
"""
...
def format_banker(val):
    # Safety check: Don't try to format None or strings
...
def render_financial_statement(df, title):
    ...
    for index, row in df.iterrows():
        ...
        if clean_index in section_headers:
            row_class = "section-header"
            is_header = True
        elif clean_index in total_rows:
            row_class = "total-row"
        elif clean_index in grand_total:
...
def main():
    ...
    view = st.sidebar.radio("Navigation", ["Executive Dashboard", "Scenario & Optimization", "Financial Statements", "Production & Sales", "OpEx Planning", "BOM & Supply Chain"])
    ...
    if view == "Executive Dashboard":
        ...
        with c1:
            ...
            inv_cash = st.number_input("Investor Equity", value=def_cash, step=100000.0)
            loc_limit = st.number_input("Bank Credit Limit", value=def_loc, step=100000.0)
            start_vol = st.number_input("Start Monthly Units", value=50)
        ...
        with c2:
            ...
            if st.button("🚀 Solve for Max Growth", type="primary"):
                with st.status("Simulating...") as status:
                    best_rate = 0.0
                    ...
                    for i in range(10):
                        mid = (low + high) / 2
                        status.write(f"Testing {mid:.1f}%...")
                        sim_u = simulate_growth_scenario(start_vol, mid, date(2026,1,1), sim_months)
                ...
                st.success(f"Max Growth: **{best_rate:.1f}% / Month**")
                if not best_cash_df.empty:
                    fig = px.area(best_cash_df, x='Date', y='Cash_Balance', title=f"Cash at {best_rate:.1f}% Growth", color_discrete_sequence=['#10B981'])
                    fig.add_hline(y=-loc_limit, line_dash="dash", line_color="red", annotation_text="Limit")
                    st.plotly_chart(fig, use_container_width=True)
        ...
        if not df_pnl.empty:
            ...
            # FIX: Explicit sum to avoid Type Error with None
        ...
        t1, t2, t3 = st.tabs(["A. Headcount & Payroll", "B. R&D Expenses", "C. SG&A Expenses"])
        with t1:
                ...
                with engine.connect() as conn:
                    ...
                    for _, r in melted.iterrows():
                        if pd.notna(r['amount']):
                            dt = date.fromisoformat(r['Month']+"-01")
                            conn.execute(text("INSERT INTO opex_general_expenses (category, expense_type, month_date, amount) VALUES (:c, :t, :d, :a)"), {"c": r['category'], "t": exp_type, "d": dt, "a": r['amount']})
                    conn.commit()
                st.success(f"Saved!")
                st.rerun()

        with tab2: render_expense_tab("R&D")
        with tab3: render_expense_tab("SG&A")
